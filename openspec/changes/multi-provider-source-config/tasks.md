...
- [x] 1.1 RED: in `config_test.go`, add cases — an entry with its own `provider` keeps it
...
- [x] 1.2 GREEN: `ParseConfig` fills the file-name provider only where `entry.Provider`
  is empty (stop overwriting non-empty values).
- [x] 1.3 RED: add `Validate` cases — a registered per-entry provider passes; an entry
...
- [x] 1.4 GREEN: `Config.Validate` loops entries, validating each by its resolved provider.

## 2. Runner.Run returns per-provider stats

- [x] 2.1 RED: in `pipeline_test.go`, assert `Run` returns `map[string]Stats` keyed by
  provider (multi-provider batch → one key per provider; per-provider Ingested/Failed
  correct; failure isolation preserved).
- [x] 2.2 GREEN: change `Run` to tally `map[string]Stats` keyed by `entry.Provider`; keep
  the single bounded concurrent pool. Update existing pipeline tests to the new shape.

## 3. cmd/ingest per-provider sweep

- [x] 3.1 RED: extend the cmd/ingest test so a run with two providers (one ingested>0, one
  ingested=0) sweeps only the first; the done-log aggregates the map.
- [x] 3.2 GREEN: `cmd/ingest` consumes `map[string]Stats`: logs the summed totals, then
  for each provider with `Ingested > 0` calls `CloseUnseenJobs(provider)`.

## 4. custom.yml + remove one-line files

- [x] 4.1 Add `sources/custom.yml` with the 13 boardless single-source providers plus
...
- [x] 4.2 Validate `custom.yml` loads + passes `Validate` against the real registry (a
